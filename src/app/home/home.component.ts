...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  sendText(data?: any): void {
    ...
    if (this.inputText.trim() !== '' || data) {
      this.messages.push({ user: 'You', text: (data ? data : this.inputText), time: new Date() });
      this.getResponse(data ? data : this.inputText).subscribe(res => {
        setTimeout(() => {
          this.dotLoader = false;
          this.messages.push({ user: 'Bot', text: res.result, time: new Date() });
